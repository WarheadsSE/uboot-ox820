...
#define DIAG_NANDRW_START	0x1c0000
#define DIAG_NANDRW_NBLOCKS	2
...
int mvNandReadWriteTest(void)
{
	...
	do
	{
		...
		er_opts.offset = DIAG_NANDRW_START;
		er_opts.length = pageSize *
				 PAGES_PER_BLOCK *
				 DIAG_NANDRW_NBLOCKS;
		er_opts.quiet = 1;
		...
		wr_opts.offset = DIAG_NANDRW_START;
		/* opts.forcejffs2 = 1; */
		wr_opts.pad = 1;
		wr_opts.blockalign = 1;
		wr_opts.quiet = 1;

		for(i=0; i < PAGES_PER_BLOCK * DIAG_NANDRW_NBLOCKS; i++)
		{
		...
		rd_opts.offset = DIAG_NANDRW_START;
		rd_opts.quiet = 1;
		...
		for(i=0; i < PAGES_PER_BLOCK * DIAG_NANDRW_NBLOCKS; i++)
		{
			...
			{
				if (buf[j] != 0x55 && buf[j+1] != 0xaa && buf[j+2] != 0x55 && buf[j+3] != 0xaa)
				{
